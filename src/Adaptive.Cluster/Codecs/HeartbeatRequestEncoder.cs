...
public class HeartbeatRequestEncoder
{
    ...
    public const ushort TEMPLATE_ID = 79;
    public const ushort SCHEMA_ID = 111;
    public const ushort SCHEMA_VERSION = 14;

    private HeartbeatRequestEncoder _parentMessage;
    ...
    public HeartbeatRequestEncoder Wrap(IMutableDirectBuffer buffer, int offset)
    {
        this._buffer = buffer;
        this._offset = offset;
    ...
    public HeartbeatRequestEncoder WrapAndApplyHeader(
    ...
    public HeartbeatRequestEncoder CorrelationId(long value)
    {
    ...
    public HeartbeatRequestEncoder ResponseStreamId(int value)
    {
    ...
    public HeartbeatRequestEncoder PutResponseChannel(IDirectBuffer src, int srcOffset, int length)
    {
    ...
    public HeartbeatRequestEncoder PutResponseChannel(byte[] src, int srcOffset, int length)
    {
        if (length > 1073741824)
        {
            throw new InvalidOperationException("length > maxValue for type: " + length);
    ...
    public HeartbeatRequestEncoder ResponseChannel(string value)
    {
    ...
    public HeartbeatRequestEncoder PutEncodedCredentials(IDirectBuffer src, int srcOffset, int length)
    {
        if (length > 1073741824)
        {
            throw new InvalidOperationException("length > maxValue for type: " + length);
    ...
    public HeartbeatRequestEncoder PutEncodedCredentials(byte[] src, int srcOffset, int length)
    {
        if (length > 1073741824)
        {
            throw new InvalidOperationException("length > maxValue for type: " + length);
    ...
    {
        HeartbeatRequestDecoder writer = new HeartbeatRequestDecoder();
