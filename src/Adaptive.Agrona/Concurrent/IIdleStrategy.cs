...
namespace Adaptive.Agrona.Concurrent
{
    ...
    /// stateless. Where implementations are stateful it is recommended that implementation state is padded to avoid
    /// false sharing.
    ///
    /// <h3>Note regarding potential for TTSP(Time To Safe Point) issues</h3>
    ///
    /// If the caller spins in a 'counted' loop, and the implementation does not include a a safepoint poll this may
    /// cause a no (Time To SafePoint) problem. If this is the case for your application you can solve it by preventing
    /// the idle method from being inlined by setting the compilation symbol IDLESTRATEGY_NOINLINE.
    ///
    /// </summary>
    public interface IIdleStrategy
    {
        /// <summary>
        /// Perform current idle action (e.g. nothing/yield/sleep). This method signature expects users to call into it
        /// on every work 'cycle'. The implementations may use the indication "workCount &gt; 0" to reset internal
        /// backoff state. This method works well with 'work' APIs which follow the following rules: <ul>
        ...
        /// <seealso cref="Reset()"/>
        /// to clear internal state when idle period is over (or before it begins). Callers are expected to follow this
        /// pattern:
        ///
        /// <pre>
        /// <code>
