        ...
        public override long Offer(IDirectBuffer bufferOne, int offsetOne, int lengthOne, IDirectBuffer bufferTwo, int offsetTwo, int lengthTwo, ReservedValueSupplier reservedValueSupplier = null)
        {
            ...
            if (!_isClosed)
            {
                ...
                if (position < limit)
                {
                    ...
                    if (length <= MaxPayloadLength)
                    {
                        resultingOffset = termAppender.AppendUnfragmentedMessage(_headerWriter, bufferOne, offsetOne, lengthOne, bufferTwo, offsetTwo, lengthTwo, reservedValueSupplier, termId);
                    }
                    else
                    {
                        CheckMaxMessageLength(length);
                        resultingOffset = termAppender.AppendFragmentedMessage(_headerWriter, bufferOne, offsetOne, lengthOne, bufferTwo, offsetTwo, lengthTwo, MaxPayloadLength, reservedValueSupplier, termId);
                    }
