...
namespace Adaptive.Aeron
{
    ...
    /// <para>
    /// <seealso cref="ExclusivePublication"/>s are created via the
    /// <seealso cref="Aeron.AddExclusivePublication(String, int)"/> method,
    ...
    /// </para>
    /// <para>
    /// <seealso cref="ExclusivePublication"/>s have the potential to provide greater throughput than the default
    ...
    public sealed class ExclusivePublication : Publication
    {
        ...
        /// <summary>
        /// Mark the publication to be revoked when <seealso cref="Publication.Dispose()"/> is called. See
        /// <seealso cref="Revoke()"/>
        /// </summary>
        ...
        /// Hence, it should be used only when it's known that all subscribers have received all the data, or if it
        ...
        /// <returns> The new stream position, otherwise a negative error value of
        /// <seealso cref="Publication.NOT_CONNECTED"/>,
        /// <seealso cref="Publication.BACK_PRESSURED"/>, <seealso cref="Publication.ADMIN_ACTION"/>,
        /// <seealso cref="Publication.CLOSED"/>, or <seealso cref="Publication.MAX_POSITION_EXCEEDED"/>. </returns>
        /// <exception cref="ArgumentException"> if the length is greater than
        /// <seealso cref="Publication.MaxMessageLength"/> framed. </exception>
        ...
        /// <seealso cref="Publication.NOT_CONNECTED"/>,
        /// <seealso cref="Publication.BACK_PRESSURED"/>, <seealso cref="Publication.ADMIN_ACTION"/>,
        /// <seealso cref="Publication.CLOSED"/>,
        /// or <seealso cref="Publication.MAX_POSITION_EXCEEDED"/> . </returns>
        /// <exception cref="ArgumentException"> if the length is greater than remaining size of the current term.
        /// </exception>
        /// <exception cref="ArgumentException"> if the first frame within the block is not properly formatted, i.e. if
        /// the
        ...
        private void CheckFirstFrame(IMutableDirectBuffer buffer, int offset)
        {
            ...
            int blockTermOffset = buffer.GetInt(offset + TERM_OFFSET_FIELD_OFFSET, ByteOrder.LittleEndian);
            int blockSessionId = buffer.GetInt(offset + SESSION_ID_FIELD_OFFSET, ByteOrder.LittleEndian);
            int blockStreamId = buffer.GetInt(offset + STREAM_ID_FIELD_OFFSET, ByteOrder.LittleEndian);
            int blockTermId = buffer.GetInt(offset + TERM_ID_FIELD_OFFSET, ByteOrder.LittleEndian);
            int blockFrameType =
                buffer.GetShort(offset + HeaderFlyweight.TYPE_FIELD_OFFSET, ByteOrder.LittleEndian) & 0xFFFF;

            if (
                blockTermOffset != _termOffset
        ...
        private int Claim(UnsafeBuffer termBuffer, int tailCounterOffset, int length, BufferClaim bufferClaim)
        {
            int frameLength = length + HEADER_LENGTH;
            int alignedLength = Align(frameLength, FRAME_ALIGNMENT);
            int termLength = termBuffer.Capacity;
