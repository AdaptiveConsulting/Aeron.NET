...
namespace Adaptive.Aeron.Exceptions
{
    public class ControlProtocolException : AeronException
    {
        ...
        public ControlProtocolException(ErrorCode code, Exception rootCause)
            : base(rootCause.Message, rootCause, ErrorToCategory(code))
        {
            _code = code;
        }
        ...
        internal static Category ErrorToCategory(ErrorCode errorCode)
        {
            return Adaptive.Aeron.ErrorCode.RESOURCE_TEMPORARILY_UNAVAILABLE == errorCode
                ? Category.WARN
                : Category.ERROR;
        }
    }
