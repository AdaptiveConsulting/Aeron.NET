...
namespace Adaptive.Aeron.Tests.LogBuffer
{
    public class HeaderWriterTest
    {
            ...
            int termId)
        {
            ...
            PutInt(_defaultHeaderBuffer, DataHeaderFlyweight.SESSION_ID_FIELD_OFFSET, sessionId, ByteOrder.LittleEndian);
            ...
            Assert.AreEqual(-frameLength, GetInt(_termBuffer, termOffset + HeaderFlyweight.FRAME_LENGTH_FIELD_OFFSET, ByteOrder.LittleEndian));
            ...
            Assert.AreEqual(headerType, GetShort(_termBuffer, termOffset + HeaderFlyweight.TYPE_FIELD_OFFSET, ByteOrder.LittleEndian));
            Assert.AreEqual(termOffset, GetInt(_termBuffer, termOffset + DataHeaderFlyweight.TERM_OFFSET_FIELD_OFFSET, ByteOrder.LittleEndian));
            Assert.AreEqual(sessionId, GetInt(_termBuffer, termOffset + DataHeaderFlyweight.SESSION_ID_FIELD_OFFSET, ByteOrder.LittleEndian));
            Assert.AreEqual(streamId, GetInt(_termBuffer, termOffset + DataHeaderFlyweight.STREAM_ID_FIELD_OFFSET, ByteOrder.LittleEndian));
            Assert.AreEqual(termId, GetInt(_termBuffer, termOffset + DataHeaderFlyweight.TERM_ID_FIELD_OFFSET, ByteOrder.LittleEndian));
        }
        ...
        public void ShouldEncodeHeaderUsingBigEndianByteOrder(
            ...
            int termId)
        {
            Assume.That(!BitConverter.IsLittleEndian, "BE-only test (skipped on LE platforms)");
            ...
            Assert.AreEqual(-frameLength, GetInt(_termBuffer, termOffset + HeaderFlyweight.FRAME_LENGTH_FIELD_OFFSET, ByteOrder.BigEndian));
            Assert.AreEqual(version, _termBuffer.GetByte(termOffset + HeaderFlyweight.VERSION_FIELD_OFFSET));
            Assert.AreEqual(flags, _termBuffer.GetByte(termOffset + HeaderFlyweight.FLAGS_FIELD_OFFSET));
            Assert.AreEqual(headerType, GetShort(_termBuffer, termOffset + HeaderFlyweight.TYPE_FIELD_OFFSET, ByteOrder.BigEndian));
            Assert.AreEqual(termOffset, GetInt(_termBuffer, termOffset + DataHeaderFlyweight.TERM_OFFSET_FIELD_OFFSET, ByteOrder.BigEndian));
            Assert.AreEqual(sessionId, GetInt(_termBuffer, termOffset + DataHeaderFlyweight.SESSION_ID_FIELD_OFFSET, ByteOrder.BigEndian));
            Assert.AreEqual(streamId, GetInt(_termBuffer, termOffset + DataHeaderFlyweight.STREAM_ID_FIELD_OFFSET, ByteOrder.BigEndian));
            Assert.AreEqual(termId, GetInt(_termBuffer, termOffset + DataHeaderFlyweight.TERM_ID_FIELD_OFFSET, ByteOrder.BigEndian));
        }
